...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth =-np.power(self.nodes[2, :], 2)        # Тут может быть скорость, но сейчас здесь нули
        self.velocity = np.array([np.zeros(len(nodes_coords[0::3])),np.zeros(len(nodes_coords[0::3])),np.ones(len(nodes_coords[0::3]))])
    ...
    def simitry(self):
        X_centre = (np.max(self.nodes[0])+np.min(self.nodes[0]))/2
        self.nodes[0]-= X_centre
        Y_centre = (np.max(self.nodes[1])+np.min(self.nodes[1]))/2
        self.nodes[1]-= Y_centre
        Z_low = np.min(self.nodes[2])
        self.nodes[2]-=Z_low
    # Метод отвечает за выполнение для всей сетки шага по времени величиной tau
    def move(self, tau):
        # По сути метод просто двигает все точки c их текущими скоростями
        self.nodes += self.velocity * tau
    def updet(self,tau,t):
        Lencoord = len(self.nodes[0])
        L=(self.nodes[1] ** 2 + self.nodes[0] ** 2)**0.5
        phi0 = np.arctan2(self.nodes[1],self.nodes[0])
        self.smth =-1*(2+(t/70))*(self.nodes[2] -np.min(self.nodes[2]))
        self.velocity[2] = 0.2*np.ones(Lencoord)*(t/30)**3
        self.nodes[2] += self.velocity[2] * tau
        self.nodes[0] = L*np.cos(phi0 + w*tau)
        self.nodes[1] = L*np.sin(phi0 + w*tau)
...
try:
    ...
    gmsh.merge(os.path.join(path, 'rocket_v2.stl'))
except:
